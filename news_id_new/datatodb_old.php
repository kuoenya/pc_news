<?php
	...
	function get_one_news($link){
		...
		echo "2/content_ ".'<br>';
		echo $content;
		// var_dump($content->find('p'));
		// exit();
		// foreach ($content->find('p') as $one_content) {
		// 	echo "<br>";
		// }
		$date = $html->find('span[class="created"]',0)->plaintext;
		echo "3/date_ "; echo($date).'<br>';

		// $praise =$html->find('span[class="_5n6h _2pih"]',0);
		// echo "4/praise_ ";  var_dump($praise)."<br>";
		// exit();
	...
	function get_praise($link){
		...
		$praise =$html->find('span[class="_5n6h _2pih"]',0)->plaintext;
		echo "4/praise_ ";  echo $praise;
		$sql_title = "INSERT INTO news_id_new (praise) VALUES 
		('".$praise."')";	
		echo $sql_title.'<br>';
		// exit();
		mysqli_set_charset($conn,"utf8");  //IMPORTANT kill 亂碼!!
